...
selectInput("borough_choice", label = h3("Select borough"),
            choices = boros, selected = "")

race = borough_final %>% pull(race_ethnicity) %>% unique()

selectInput("race_choice", label = h3("Select Race"), choices = race, selected = "")
```
...
renderPlotly({ 
  borough_final %>%
  ungroup() %>% 
  filter(borough == input$borough_choice, race_ethnicity == input$race_choice) %>% 
  group_by(year, sex) %>% 
